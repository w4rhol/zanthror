...
zbmicat_stata <- function(bmi, age, gender, male_code = 1, female_code = 2, age_unit = "year", wtabbr = FALSE, return = "string") {
  ...
  # Load the IOTF reference data (cache it for performance)
  if (!exists(".iotf_data_cache", envir = .GlobalEnv)) {
    if (!file.exists("iotf.csv")) {
      stop("iotf.csv file not found. Please ensure the IOTF reference data file is in your working directory.")
    }
